...
type Props = {
  ...
  /** Render as a full-pane workbench instead of a floating overlay. The
   *  Library uses this so the detail view IS the page (no backdrop, fills
   *  its parent cell, no close button / Esc-to-close). Coverage + Roster
   *  leave it unset for the classic click-to-open modal. */
  inline?: boolean;
};

/**
 * Lens — the reusable photo viewer + operations surface.
 *
 * Two render modes (see `inline`):
 *   • overlay (default): a floating modal — click-to-open from Coverage /
 *     Roster grids. Backdrop + close button + Esc-to-close.
 *   • inline: a full-pane workbench — the Library renders this as the whole
 *     no-scroll page (no backdrop / close / Esc).
 *
 * Layout (both modes):
 *   left pane:  ONE preview that swaps between the original photo, the
 *               Tesseract-prepared OCR view (with bbox overlays), and the
 *               face-overlay view. A thumbnail strip below picks siblings.
 *   right pane: bibs · faces · cluster · metadata · storage · actions ·
 *               OCR + face debug (owners get the OCR tuning knobs).
 *
 * Keyboard:
 *   Esc            → close (overlay mode only)
...
export function Lens({
  ...
  const grid = (
      <div
        ...
        style={{
          ...
          // never gets clipped by the bottom of the page. Inline (the
          // Library workbench) fills its parent cell instead.
          height: inline ? "100%" : "92dvh",
          maxHeight: inline ? "100%" : "92dvh",
        }}
      >
  ...
  // Inline (Library workbench): no backdrop — the grid fills its parent.
  if (inline) {
    return (
      <>
        {grid}
  ...
  // Default: floating overlay (Coverage / Roster click-to-open).
  return (
