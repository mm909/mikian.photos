...
/* ABOUT YOU — the rower's own birthday, height and weight (owner,
 * 2026-09-24: "collect their birthday … height and weight optional, in the
 * settings page"). PATCH { birthday?, heightCm?, weightKg? }: a key that is
 * absent is left alone; null or "" clears it; anything else must parse.
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  let body: { birthday?: unknown; heightCm?: unknown; weightKg?: unknown };
  try {
  ...
  const data: { birthday?: Date | null; heightCm?: number | null; weightKg?: number | null } = {};
