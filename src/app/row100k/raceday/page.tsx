 ...
 * code), THE ACT (sign in / opt in / put my name in / you are in), and THE
 * RACERS — the field as a board table, one bracket at a time.
 *
...
export default async function RaceDayPage() {
  ...
  const headNote =
    phase === "open" ? "FREE TO ENTER" : phase === "closed" ? "REGISTRATION CLOSED" : "RACED";
      ...
      <section>
        <div className="wrap">
          ...
          <div className="sec-head">
            <h2>{race.title}</h2>
            <span className="mono">{headNote}</span>
          </div>
          ...
          <div className="rd-stub">
            ...
            <ul className="rd-facts">
              ...
              <li>
                ...
                <span className="v">{race.meters.toLocaleString("en-US")} m, timed</span>
              </li>
              <li>
                <span className="k">Entry</span>
                <span className="v">Free</span>
              </li>
              {/* The gym's waiver, said out loud among the facts rather than
               * sprung at the end (owner sent the link 2026-09-11). */}
              {race.waiver && (
                <li>
            ...
            <p className="rd-blurb">{race.blurb}</p>
            <p className="rd-closes">{race.closesLine}</p>
          </div>
      ...
      <section>
        <div className="wrap">
          <div className="sec-head">
            <h2>The racers</h2>
            <span className="mono">
              {field.length === 0 ? "NOBODY YET" : `${field.length} IN THE FIELD`}
            </span>
          </div>
          ...
          <p className="rd-foot">
            Fastest 5k is the best one on the September board, rowed or converted from a longer piece.
            <br />
            Waves go out by email. {race.venueLine} · {race.when}
          </p>
        </div>
      </section>
