 ...
 * place as a row of its own (.mlg-editor, theme.ts, for the inputs; the
 * mono caps labels over its fields are here, .lgt-fld).
 *
...
/* The rail: the dots sit a step in from the right edge of the tinted
 * table (owner, 2026-09-25: the dots were too close to the border, some
 * buffer there) — the cell keeps the dots on its right with 10px of paper
 * after them, and is wide enough that the padding costs the numbers
 * nothing. */
.row100k table.board.lgt th.lgt-c,.row100k table.board.lgt td.lgt-c{width:40px;padding-left:0;padding-right:10px;text-align:right}
...
/* THE EDITOR LABELS (owner, 2026-09-25: on the edit screen, use labels
 * for date, meters, time): each field is a block with its mono caps label
 * over the input — the log form label face (label.fl, theme.ts) at the
 * table size. The date field is as wide as a native date needs; meters
 * and time keep their 90px. */
.row100k .lgt-fld{display:block;min-width:0}
.row100k .lgt-lab{display:block;font-family:var(--row-mono),monospace;font-size:10px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--gray);margin-bottom:4px}
.row100k .mlg-edit-line{align-items:flex-end}
.row100k .lgt-edit .mlg-edit-split{padding-bottom:6px}
...
@media (max-width:560px){
  ...
  .row100k table.board.lgt th.lgt-c,.row100k table.board.lgt td.lgt-c{width:30px;padding-right:8px}
