...
- community print: TIME ROWED "953.1 h" · SESSIONS "1,686" · ROWERS "96" · 100K FINISHERS "25". Phone: three cells TIME ROWED · ROWERS · 100K FINISHERS. (The tier's title is the owner's, 2026-09-10: "instead of saying IN THE 100K CLUB, I kind of like the title of 100K FINISHERS" — it is the same words on the ledger line.)
...
**C5 `curve`. OFF EVERY PLAN** (owner, 2026-09-10: "the curve needs to go. I do not really like the whole curve there with the cumulative meters"). Kept registered in `community.ts` and whole in `charts.ts`. What it draws when a plan names it: eyebrow "THE CURVE" / "METERS TOGETHER · SEP 1 → SEP 30" (`→ SEP N`). Cumulative meters Sep 1 → as-of day (the frame is the elapsed days). Left gutter 4.2 × axis for y labels; **nice-step** gridlines: step = the first of `pow, pow/2, pow/4, pow/5, pow/10` (pow = 10^floor(log10(max))) giving 4–5 lines, labelled "500K", "1M", "2.5M"; top line solid, others dashed grid; ink baseline; x ticks "SEP 1" then bare numbers per `dayTicks(span)` (≤ 10 days: every day); water area + line, end dot, end label mono 700 small `fmtMeters`. Draws when `dayNumber ≥ 2`, else the eyebrow plus a mono line "THE CURVE STARTS TOMORROW". Height = whatever the plan hands it (min 210 printL / 160 printS / 140 phone; max 320 / 220 / 220). Chart-only height is the box minus the eyebrow.

**C6 `month` / `month.full`.** Eyebrow "THE MONTH" / "METERS PER DAY". Seven columns S M T W T F S (mono axis gray letters), Sep 1 in the Tuesday column (two blanks). Since the curve came off, THE MONTH IS THE PICTURE of every sheet — two columns wide on the 24x36 — so the module carries three rules the small version never needed:

- **Two registrations.** `month.full` is the whole September grid (five rows whatever the day) and is what every PRINT plan uses, so a wall sheet composes identically at day 12 and at FINAL — item 17 ("make sure it still looks good when the month chart is filled") is then true by construction. `month` is the elapsed weeks only and is what the phone frames use: they are shared the day they are made.
- **Future days are drawn** (`full` only): an empty box in `grid` (one step lighter than a rest day's dashed `line`) with its date in `line`. Three blank rows of September under a big calendar read as a hole; an empty box reads as a month in progress.
- **The cell fills its box.** The width is always the column's share — `cellW = (w − 6 × gap) / 7`; the HEIGHT is `min(cellCap, cellW)`, so a cell is square wherever the box is deep enough (every print sheet, and a phone frame mid-month) and a wall-calendar rectangle where it is not (a FINAL five-row month on a 4:5 post), instead of a square grid in a 40 %-empty box. `community.ts` computes the cap from `box.h`: the module MEASURES at the width cell on print and at a 34-unit floor cell on the phone, and the plan's `grow` hands it the rest. Under a small cell the k-label goes (`cell × .28 < small × .8`) and then the day number (`cell < axis × 1.8`) rather than print mud.

Rest days a dashed `line` outline, rowed days a ramp cell with the day number top-left (mono axis, ink; white on the top bucket) and the kLabel ("312k", "1.2M") bottom-centre in mono 700. Gap = 0.7 × small. Community buckets: **quartiles of the elapsed days' meters** (§14.1). Height = eyebrow + axis row + rows × cellH + (rows − 1) × gap.
...
**C8 `records`.** Eyebrow "THE RECORDS" / "THIS SEPTEMBER" (blackout: "TIMES ARE SHOWN, HIDDEN METERS ARE NOT", short "TIMES SHOWN"). Compact list: small gray label (FASTEST 5K, FASTEST 10K, LONGEST ROW, BIGGEST DAY), then one line per holder: division letter gray, value in water Archivo Black recV (a shape → `paint.figure` blocks at recV with " m" at row), holder Archivo 700 row on the same baseline, "· 075 · SEP 4" small gray; dashed hairline between records. Six lines. The line pitch levels the list with the boards' ten rows beside it — and, when a plan hands the module a TALLER box (the room the curve used to take; the records were the owner's keep and the curve was not), the lines spread to fill it, up to `recV × 1.5`, so the column closes with its neighbours instead of ragging. `measure()` runs unbounded, so the natural pitch is what the engine budgets.
...
Re-composed 2026-09-10 evening: with the curve, the club roll and the partner plate off every plan and the footer down to one line, the sheet is the number, the strip, THE MONTH, the two top tens, the records, the hours, the field and the takeaways ledger — and every plan below was drawn again around the calendar rather than patched. Measured heights on the fixture (printL, day 12 = FINAL for `month.full`): masthead 143 · headline 161 · strip 113 · month.full 558 at span 2 / 307 at span 1 · board.10 307 · records 307 natural (fills to its box) · hours 170 · field 170 · ledger 95 at span ≥ 2 (two columns) / 176 at span 1 · footer 42.

**tall** (24x36 first plan) — `masthead · headline · strip · D[{month.full, ledger} ×2 fit:lines | {board.men, board.women} ×1] · E[{hours, field} ×2 | records ×1] · footer`; shrink `gap, pitch, cap`; next `short`. Two flows under the strip: the calendar two columns wide with the takeaways under it (683) against the two top tens down the right rail (644), then the hours over the field (370) beside the records, which fill the same height. Slack 0, no shrink step spent, and the same composition at day 1, at day 12 and at FINAL — only the calendar's fill changes. A blackout's second dateline line (+22) is paid for by the ledger yielding two of its six lines.

**short** (18x24 first plan; 24x36 fallback) — `masthead · headline · strip · D[month.full | board.men | board.women] · E[{hours, field} ×2 | records ×1] · F[ledger ×3 fit:lines] · footer`; shrink `gap, pitch, cap`; next `squat`. 200 units shorter than the 24x36, so the month gives up a column and stands beside the two boards (307 all three), and the takeaways run the full measure across the foot as one wide dotted ledger. Slack 0, no shrinks.

**squat** (16x20 first plan; 18x24 fallback) — the same rows as `short`; shrink `gap, pitch, cap`; next `core`. 100 units shorter again: it spends the `gap` step and holds everything, because every block on this sheet is one the owner asked to keep.

**core** (printL terminal) — `masthead · headline · strip · D[board.men.5 | board.women.5 | records] · E[ledger ×3 fit:lines] · footer`; shrink `gap, pitch, cap`; no next (last resort, §8.6).

**hand** (11x17, A3, letter, A4; two columns) — `masthead · headline · strip · D[{month.full, ledger} ×1 fit:lines | {board.men.5, board.women.5} ×1] · footer`; shrink `gap, pitch, cap`; terminal. The whole front page above the fold: the month and the takeaways in the first column, the two top fives in the second. The ledger is the elastic part — 11x17 holds all six lines (slack 23), A3 and A4 four (slack 0, `gap`), letter two (slack 0, `gap, pitch`). A printS column is 302 units: twenty board rows and a calendar cannot share it, so the hand-outs keep the top five they were designed with.

**story** — `masthead · headline · strip · D[month ×2 fit:cap] grow 1 · F[ledger.phone ×2 fit:lines] · footer`; shrink `gap, cap`; terminal. The month is the picture. Mid-month it grows to square 61-unit cells over four takeaways; at FINAL the five-row grid takes the room the ledger yields (two lines) and its cells go wall-calendar wide.

**post** — the same rows as `story` (the 4:5 frame is simply shorter): mid-month a two-row month at 34-unit cells over two takeaways; at FINAL the ledger yields whole and the full month fills the frame.

**square** — `masthead · headline · strip · F[ledger.phone ×2 fit:lines] · footer`; shrink `gap, cap`; terminal. 1:1 has room for the number, the strip and three or four takeaways and nothing else — a calendar under them would be four rows of 20-unit cells.
...
3. **Club roll past ~45 names on the 24x36** — MOOT since §16: the roll is off every plan ("there is going to be a ton of members eventually"), which is the owner answering this question himself.
...
- **§6 C6** — the September calendar is registered twice in `community.ts`: `month.full` (five rows whatever the day, future days as empty `grid` boxes) is every PRINT plan's month; `month` (elapsed weeks only) is the phone's. `ledger.side` is gone with the old short plan — the takeaways are a full-width row there now. `charts.ts drawMonth` (community, quartile buckets, cellW / cellH) and `rowerCharts.ts drawMonth` (rower, fixed 2.5k / 5k / 10k, DAYS ROWED masked form) are two drawings of one grid; folding one into the other is a later pass.
- **§7 community, all plans** — re-composed after the owner's review (§16). No community plan spends `chart` or `step` any more: the curve is off every plan and the club roll with it, so the steps they belonged to are gone from the shrink orders (`gap, pitch, cap` on paper, `gap, cap` on the phone). §14.3 (the roll past 45 names) is moot for the same reason.
- **§7 community story / post / square** — no drop list: the engine spends drops BEFORE shrink steps, and a ledger dropped whole leaves a hole where the `cap` step keeps two lines of it. All three shrink `gap, cap`. The month is a `fit: "cap"` slot in a `grow` row and measures a 34-unit floor cell, so it takes whatever the ledger leaves and a FINAL five-row month shrinks its cells (and then its labels) rather than push the footer.
...
Every line here is a decision, not a suggestion; `community.ts` and `charts.ts` carry the quotes where the call was his.

1. **The day count is off both sheets.** The dateline is "SEP 12" (or "FINAL" from Oct 1) and the headline label ends at "· 100 ROWERS": "if it is September tenth, then we know it is day ten." Nothing on either sheet prints a day count.
2. **The footer is one line** — FOR YOURSELF AND OTHERS, mono gray uppercase under the thick rule. The bold name, the URL, the handle and the partner line are gone (`charts.ts drawFooter`, shared with the rower sheet).
3. **No partner anywhere.** The plate is off every plan; the module and the payload field stay for a future partner.
4. **The curve is gone** from every plan, and **the 100K club roll** with it; the count survives as a strip cell and a ledger line, and the strip's title is now **100K FINISHERS**. The takeaways' club line lost its "FIRST <NAME> · <DAY>" tail.
5. **The month is the picture.** Two columns on the 24x36 (95-unit cells), one on the 18x24 and 16x20, one on the hand-outs, the full frame on a phone; five rows all month long on paper, with the days still to come drawn as empty boxes.
6. **Kept, in his words**: "I like that we list the top ten men and women. I like that we have the hours and the field chart here. I like that we call out the records here for fastest rows, longest row, biggest day." All four are on every wall sheet; the hand-outs keep the top five they were designed with (a 302-unit column cannot hold twenty rows and a calendar).
7. **A filled month composes** (item 17): because `month.full` is always five rows, the 24x36, 18x24 and 16x20 draw the SAME sheet on day 1, on day 12 and at FINAL. The phone frames are measured at a floor cell and grow, so a FINAL story keeps two takeaways and a FINAL post gives the whole frame to the month.

Offered, not taken (the owner's call): the two boards could run deeper than ten on the 24x36 — the right rail has room for about three more rows a side if he wants a top thirteen — but "do not silently change what is listed", so they stay at ten.
