...
/**
 * /orders — a signed-in person's purchase history. Lists orders linked to this
 * account by either:
 *   - `userId` match (linked at capture time when the buyer was signed-in)
 *   - email match (catches the case where someone bought as guest then signed
 *     in later with the same email)
 *
 * Not signed in → /api/auth/signin with callback back here. (Replaces the old
 * /runner route — galleries aren't always races.)
 */
...
export default async function OrdersPage() {
  ...
  if (!actor) {
    redirect("/api/auth/signin?callbackUrl=" + encodeURIComponent("/orders"));
  }
